...
### Colour: catalog corrected + manual override added — done, 2026-09-06
...
### Step 9 rebuilt properly — this is core, not optional — done, 2026-09-06

Monica was clear the first version (solid-colour capsules) missed the actual point of Step 9: she wants to see the outfit *worn*, and called out five specific, concrete problems after looking at it. Rebuilt to genuinely address all five, verified against the real running app after each fix (not assumed from reading the code):

1. **"I can't see its head at all."** Root cause: the camera position/FOV were fixed numbers guessed in advance, and the figure's actual height didn't fit inside that frame. Fixed properly, not by nudging numbers until one photo happened to fit: the camera now measures the figure's real bounding box every time (`fitCameraToFigure()`) and backs off exactly far enough that head-to-feet and full width both fit, recalculated whenever a garment image finishes loading. This can't go stale the way a hand-tuned camera position could.
2. **"It's only pairing with the first recommendation."** / **3. "What if a user wants the 2nd or 3rd top?"** The mannequin preview used to always show `result["matches"][0]`. Added a "See it paired with:" dropdown listing every currently-filtered match by name and price - picking a different one re-renders the mannequin with that item's colour. Verified with a real browser test: opened the dropdown (confirmed all matches listed, e.g. "Relaxed Fit Top ₹949", "Fitted Cotton Shirt ₹1599", "Beige Blazer ₹3199"...), selected a different one, and confirmed via screenshot the torso colour actually changed to match the new selection.
3. **"The mannequin is not properly showing and make it more human like not a balloon."** The first version was three oversized capsules (head/torso/legs) with no neck, shoulders, hands, or feet - genuinely balloon-like. Rebuilt with a proper segmented figure (feet, shins, thighs, hips, torso, shoulders, upper arms, forearms, hands, neck, head), each segment's position computed from the one below it so there are no gaps or floating pieces. Styled deliberately as a neutral wooden artist's mannequin (one consistent tone, not attempted skin realism) rather than trying and falling short of photorealism - an intentional, explainable design choice, not a corner cut.
4. **"Make that mannequin area look like a studio, not a scrap project."** Added a soft vertical-gradient backdrop (drawn on a canvas at runtime, like photography sweep paper), a soft blob-shadow under the feet, and three-point lighting (key + fill + rim) instead of one flat light - the figure now reads as a lit 3D object in a space, not a shape floating on a blank background.

**A real, separate finding surfaced during this rebuild, not anticipated in advance:** the original plan was to texture the *actual matched catalog photo* onto the mannequin (a literal "wearing the clothes" look). Testing this for real showed why that doesn't work well here: this catalog's photos are lifestyle/mirror-selfie shots of a model wearing a full outfit (see `test_12_shirt.jpg` - shows the "Everyday Cotton Shirt" but the same photo also has the model's arm, a phone, a shoulder bag, and a completely different pair of brown pants). Background removal only strips the plain backdrop; it doesn't know to exclude the bag or the model's arm or the other garment in frame - texturing the raw cutout put a messy collage of unrelated objects on the mannequin, exactly matching Monica's "looks like shit" reaction to the first attempt.

**Decision, made with Monica rather than guessed:** the customer's own uploaded photo is shown as its real, textured cutout (they just cropped it themselves with the Step-8B crop feature specifically for this purpose, so it's clean), while the matched catalog item is shown as a smooth colour panel instead of its source photo. This isn't a downgrade forced by a missing feature - it's the more honest choice given what the catalog photos actually are, and it was Monica's explicit pick from three options (real photo for both and accept the mess; manually re-shoot/crop all 34 catalog photos first; or this hybrid) after seeing the actual problem.

**Verified, step by step, against the real running app - not assumed from the code:** confirmed the full head-to-feet figure renders and fits in frame; confirmed the garment panel meets the mannequin's waist/hip cleanly (an earlier pass left a bare, un-textured hip segment showing between the shirt panel and the jeans panel - fixed by anchoring each panel from its natural hang-point, shoulder for a top and waist for a bottom, so a shorter panel loses length at the hem rather than leaving a gap at the join); confirmed picking a different match from the dropdown actually changes the rendered colour; confirmed the studio backdrop, floor shadow, and three-point lighting all render as intended.
...
### Step 9 pivoted away from 3D entirely, after Monica showed real reference images — done, 2026-09-06

Monica sent two reference images: a photorealistic CGI mannequin render, and a screenshot of CLO3D/Marvelous Designer (professional fashion CGI software with digital sewing patterns, physics-based cloth draping, and ray-traced rendering - the tools real fashion brands pay for and spend real time on, per garment). She was direct: the hand-built Three.js figure "looks like shit" and isn't something she can show anyone.

**Assessed honestly rather than attempting a fourth 3D iteration:** that reference quality is not reachable by a hand-coded Three.js scene in a browser at any reasonable effort level - it's not a tuning gap, it's a completely different category of tool (real cloth simulation vs. primitive shapes with a flat texture stuck on front). Explained this plainly to Monica rather than promising another round that would hit the same wall, and gave her the real choice: keep tuning the 3D figure (would still never approach the reference), drop Step 9 entirely, or replace it with something achievable and still genuinely presentable. She picked the third option.

**Rebuilt as a 2D "outfit pairing" card - no 3D, no mannequin body at all.** Shows the customer's own uploaded photo next to the matched catalog item's real photo (exactly the photo already used everywhere else in the app - Shop page, results list), in a bordered card with a "+" between them and captions underneath. This sidesteps every problem the 3D attempt had: no camera-framing math, no proportions to get right, no risk of looking like "a balloon", and no messy-cutout problem either (see the finding two entries up) - it's just two real, already-trusted photos shown side by side, which is a completely normal, expected way to present a styled pairing (the same idea as a lookbook or outfit mood-board). Verified with a real browser test after building it: the card renders cleanly with the uploaded jeans photo and the selected match's photo side by side, captioned correctly.

**Removed, not left dead:** `mannequin_view.py` and `color_swatches.py` (deleted via `git rm` - the whole 3D approach, not just patched), the `nobg_image_bytes` field `analyze_uploaded_photo()` was keeping around specifically for 3D texturing (reverted `matching_engine.py` to its leaner original behaviour), and `catalog_nobg_image_path()` from `shop_utils.py` (no longer called by anything). The "See it paired with:" match-selector dropdown from the 3D version was kept - that part of the feedback (preview any match, not just the top one) was a real, correct ask independent of 2D vs. 3D.

**A further idea raised, being investigated separately:** Monica asked whether the garment itself (not the whole photo) could be extracted cleanly for a 2D placement - i.e. proper garment segmentation, not just background removal. This is worth being upfront about: this exact idea (isolating a garment from a person wearing it) was already tried once in this project back at Step 2B, using rembg's `u2net_cloth_seg` model, and was deliberately abandoned - it failed on flat-lay photos with no person to anchor to, unusual poses, and non-standard silhouettes (see Section 2 and the "Plain background removal over cloth segmentation" entry in Section 8). Repeating that exact technique would predictably hit the same wall. Currently investigating whether a *different*, purpose-built clothing-parsing model (not the same one that already failed) can do better, tested honestly against the same real photos that broke the first attempt before any decision is made to use it - see the next entry once that investigation is complete.

### Portfolio-hardening pass: evaluation harness, tests, CI, 3x speedup — 2026-09-06

Monica asked for changes that would make the project stand out to someone reviewing it. Reading the whole codebase first turned up three things that were actually broken, then four substantial additions.

**Broken, found by reading rather than assuming:**
1. `accuracy_check.py` could not run at all - it called `analyse_color(path)`, a function that was never defined or imported anywhere (the file imports `get_dominant_color`), so it would `NameError` on its first photo. Its label list was stale too (still had "jacket" and "t-shirt", which stopped being categories; missing pants/shorts/top/blazer; and not using the tuned prompt wording). And despite the name, it never compared anything to ground truth - it just printed predictions for a human to eyeball. Deleted and replaced by `evaluate.py`.
2. `test_pipeline_end_to_end.py` appended `test.jpg` and `test2.jpg` to its input list. Both are gitignored scratch files that don't exist in a fresh clone, so anyone else running it got two guaranteed failures that looked like real pipeline bugs. Fixed, and moved to `scripts/` (see below).
3. **The pipeline encoded the same photo with CLIP three separate times per upload** - once for category, once for style, once for the FAISS embedding - because `classify_garment()` ran a full model forward pass per label set. The `[TIMING]` logs showed this was ~12s of a ~15s upload.

**Fix 1 - one CLIP pass instead of three.** Added `classify_from_embedding()` to `classify_garment.py`: it scores an already-computed image embedding against text embeddings, which is what CLIP does internally anyway. Text embeddings only depend on the label wording, which never changes, so they're now computed once per process and cached (`functools.lru_cache`). **Verified before replacing anything**, not assumed: ran both implementations on the same photo and confirmed identical ranking with a maximum probability difference of 0.000001, and confirmed `get_image_embedding()` returns the same vector the classifier uses. **Measured result: a first upload went 14.8s → 10.3s, and a second upload in the same process 14.8s → 3.2s** (category and style classification now measure 0.00s each, since they're a matrix multiply against cached text embeddings). This matters beyond the number - a visitor was previously waiting 15 seconds per upload.

**Fix 2 - a real evaluation harness (`evaluate.py`).** Scores automatic detection against the catalog's own human-assigned labels across all 49 items, reports per-category accuracy, lists every individual miss, and compares CLIP's confidence when right vs wrong. **Deliberately does not score style**: style in `catalog.json` was computed by the same CLIP classifier that would be under test, so scoring it would measure the classifier against itself and return a meaningless 100%. Saying that out loud in the script is more useful than publishing a fake number.

**What the evaluation immediately revealed - the reason it was worth building:** the previously-quoted accuracy figures were measured on small, favourable subsets and no longer hold. Category is **67.3% (33/49)**, not the 79.4% recorded earlier - that number was measured when the catalog was 34 items, and the 15 photos added on 2026-09-06 are much harder (corsets and camis that CLIP confidently reads as dresses). Failure is very unevenly distributed and now precisely known: dress, blazer, jeans, shirt, hoodie and pants all score 100%, while `top` is 7/17 and `shorts` is 1/3. Colour was measured across the full catalog for the first time and came out at **44.9% (22/49)** after the fix described below - the previously-quoted "~65%" was measured on a favourable 16-item subset chosen by filename hints, not the whole catalog. Confidence turned out to be a genuinely usable signal: CLIP averages 78.6% confidence when correct and 64.7% when wrong.

**Both numbers being lower than what was previously written down is the point, not a setback.** The earlier figures weren't wrong when they were recorded - they were measured on smaller, easier sets. Having a harness that re-measures everything on demand means the project now reports what is actually true today rather than the best number ever recorded, and it can prove it with `python evaluate.py`.

**Fix 3 - a test suite (54 tests) and GitHub Actions CI.** `tests/` covers the pure logic: pairing rules (a bottom must never pair with another bottom; colour scoring must be symmetric or rankings would depend on upload order; scores must stay within the 0-2 range the re-ranker's `/2` normalisation assumes), colour naming (including the exact 20,000-random-value check that guards the "beige" crash), the UPI link (an amount that doesn't match the displayed price is worse than a broken button), and the explanation fallback. Modules that download CLIP on import are deliberately excluded so CI stays fast. The workflow installs the real `requirements.txt` on Python 3.12 Linux, which means **CI would have caught both of this project's real deployment outages** (the torch version that only existed on PyTorch's own wheel index, and `opencv-python` needing system graphics libraries) before Streamlit Cloud did. `pytest` was added to a separate `requirements-dev.txt` so nothing test-only ships to the deployed app.

**A real bug the tests found immediately.** A test asserting "every reference swatch should name itself" failed: `closest_color_name((150, 150, 150))` - pure mid-grey, the exact grey reference value - returned **"beige"**. Cause: at zero saturation the hue reading is meaninglessly 0, which the warm-hue check read as warm, so every mid-to-light neutral grey was labelled beige. Fixed by requiring a trace of real saturation before beige can win.

**And the honest part: the first version of that fix made measured accuracy worse, and measuring caught it.** The fix changed two things at once - the beige rule (correct) and the grey/white brightness boundary (0.55 → 0.85). Re-running `evaluate.py` showed colour accuracy had *dropped*, so instead of keeping it, the two runs were diffed item by item: the change fixed one genuinely grey top but turned two genuinely white shirts grey. Measuring the actual HSV values of those three photos showed the boundary has to sit between v=0.608 (the grey top) and v=0.706 (the darker white shirt) - so 0.85 was too high and the original 0.55 too low. Set to 0.65 and recorded as `WHITE_BRIGHTNESS_THRESHOLD` with the measurements in the comment, so the next person doesn't re-tune it by feel.

**Two features added while the plumbing was open:**
- **Score breakdown per match.** `rerank()` was computing `similarity×0.5 + colour×0.35 + silhouette×0.15` and throwing the components away. They're now kept and shown in an expander under each match, with the colour and silhouette relationship names - so the ranking is inspectable rather than a black box.
- **Low-confidence prompting.** `classify_uploaded_item()` already computed a confidence score and discarded it. It's now returned and surfaced: below 70%, the app tells the customer it isn't sure and suggests checking the category. The threshold is set from the measured 78.6%-vs-64.7% gap rather than picked by feel.

**UI pass, same session.** Monica said the interface looked boring and wanted it to feel professional. Added `.streamlit/config.toml` with a warm neutral palette (`#8A6552` primary against an off-white `#F6F3F0`) suited to a clothing store rather than Streamlit's default red. **Only long-standing theme keys are used, deliberately**: Streamlit refuses to start on an unrecognised config option, and this file ships to Streamlit Cloud, which may run a different version - a newer-only key would take the live app down rather than being ignored. Verified the app boots with the config before committing it. The Shop page gained category filtering, price sorting, bordered product cards, thousands-separated prices (`₹1,499` rather than `₹1499`) and a category/colour caption. Try It On was restructured into numbered steps (1. Upload, 2. Check what we detected, 3. What it pairs with), with the crop preview beside the cropper instead of below it, detection results in a bordered panel, and each match as a bordered card with its rank, price and attributes.

**Screenshots for the README** were captured by driving the real running app with Playwright, not mocked up, and saved to `docs/screenshots/`. Reviewing them caught a copy bug the tests hadn't: the new confidence message read "Detected **a jeans** — 87% confident". Fixed by moving plural handling into `pairing_rules.describe_category()`, next to the category vocabulary itself, so the app's copy and the explanation text can't disagree - and that revealed `explanation.py` had the same latent bug, special-casing only "jeans" and so producing "This pants pairs well with your top". Both covered by tests now.
...
The garment-segmentation idea is **not** being pursued for now: the specific model rembg offers for it (`u2net_cloth_seg`) was already tried and abandoned at Step 2B for documented reasons (fails on flat-lays with no body to anchor to, on unusual poses, on non-standard silhouettes), and a purpose-built clothing-parser alternative needs `torchvision` plus a second segmentation model loaded alongside CLIP - which is hard to justify against the ~725MB/1GB memory budget. Worth revisiting only if the memory work in Planned Improvements happens first.
...
### 2026-09-06 — The actual answer to "can we do better": let a human confirm/correct it, both in the catalog and live in the app

Monica asked the obvious, better question after the ceiling was demonstrated: "can we at least manually tell the app our clothes colours?" Yes - and this is a real, substantial improvement, not a workaround.

**Part 1 - the catalog's 34 colours, verified by actually looking at every photo.** Rather than continue tuning an algorithm against a ceiling that four separate experiments had already confirmed, every catalog photo was opened and its colour set by direct visual judgement - the one thing this whole exercise proved a small nearest-neighbour palette structurally cannot do as well as a human (or an AI with vision) just looking at the image. **17 of the 34 catalog colours were wrong and got corrected** (e.g. `denimlightwashblue_skirt.jpg` was "pink", corrected to "blue"; `test_8_kurta.jpg` - the exact photo behind Monica's original pants-vs-skirt-style complaint pattern - was "orange", corrected to "yellow"; `test_05_Skirt.jpg` was "cream", corrected to "pink"). Checked against the 16 catalog items whose filenames state their real colour: **16/16 (100%)**, comfortably past the 14/16 target, because this sidesteps the algorithmic ceiling entirely rather than trying to out-tune it.

**A real risk caught before it could bite:** `build_catalog_embeddings.py` unconditionally overwrote the `color` field on every run - rerunning it later (e.g. when new catalog items are added) would have silently wiped out today's manual corrections back to the same imperfect algorithm. Fixed by adding a `color_verified` flag to every item; the script now skips recomputing colour for any item that has it, while still auto-computing a first-pass colour for genuinely new items that don't. Verified the fix actually works by rerunning the build script and confirming the corrected colours survived, not just assumed from reading the code.

**Part 2 - a confirm/correct step for real customer uploads, since the catalog fix doesn't help a live photo the app has never seen before.** `matching_engine.py`'s `find_matches()` was split into two functions: `analyze_uploaded_photo()` (the cheap detection - background removal, CLIP category, colour, CLIP style) and `find_matches_from_details()` (the expensive part - FAISS retrieval and re-ranking), so the app can show the customer what it detected and let them fix any of the three fields via dropdowns *before* the expensive retrieval/explanation step runs, rather than committing to a possibly-wrong automatic guess. `app_pages/try_it_on.py` now shows the uploaded photo, three editable dropdowns (category/colour/style) pre-filled with the detection, and a "Find my matches" button - matching only runs (and live Gemini explanations only get generated) once the customer has confirmed or corrected the details, not on every upload automatically.

**Verified with a real headless-browser test, not just reading the code:** uploaded a real photo, confirmed the three dropdowns appeared correctly pre-filled with the detected values, then actually changed the category dropdown from the detected "dress" to "jeans" and clicked "Find my matches" - the results and generated explanation text genuinely reflected "jeans" as the category (11 matches instead of the dress's 1, explanation text referencing "your ... jeans"), while colour and style stayed at their original detected values since those weren't touched. This proves the override is real - the app doesn't just relabel the display, it actually re-runs matching against the corrected value.
...
| 9 (stretch) | Show the paired outfit | A 2D "outfit pairing" card — the customer's own uploaded photo next to the matched catalog item's real photo, plain Streamlit layout (`st.container(border=True)` + columns), no 3D | A 3D mannequin was attempted first (procedural Three.js humanoid) but Monica compared it directly to professional fashion CGI (CLO3D) and found it unacceptable — that fidelity needs real cloth-simulation software, not a hand-coded browser scene, so continuing to tune it would not have closed the gap. The 2D card achieves the same goal (show the outfit together) using photos already trusted elsewhere in the app, with no proportion/rendering risk at all |
...
| — | Measure how well it actually works | `evaluate.py`, scored against the catalog's human-assigned labels | Almost no portfolio project reports measured accuracy. Knowing *where* it fails (`top` 41%, `shorts` 33%, six categories at 100%) is a stronger signal than a single headline number, and it immediately corrected two figures that had been quoted from smaller, easier subsets |
| — | Protect the logic that encodes the idea | `pytest` (54 tests), GitHub Actions CI on Python 3.12 Linux | The pairing rules *are* the project's argument, so they're what's worth testing. CI installs the real `requirements.txt`, which means it would have caught both real deployment outages (the torch wheel index, and `opencv-python` needing system graphics libraries) before Streamlit Cloud did |
| — | Make the interface look considered | `.streamlit/config.toml` theme, bordered cards, numbered steps | A working pipeline behind a default-looking UI reads as unfinished; only long-stable theme keys are used because Streamlit refuses to boot on an unknown config option and this file ships to production |
