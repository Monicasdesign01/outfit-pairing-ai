...
| 9 | Stretch — 3D mannequin | done, rebuilt after real feedback — segmented studio-lit figure, real garment photo for the upload + colour panel for the match (data-quality finding, see Section 11), match-selector dropdown for any recommendation, not just the top one |
...
| 9 (stretch) | Show the outfit on a 3D figure | Three.js, procedural segmented humanoid (no `.glTF` file), studio backdrop + floor shadow + three-point lighting, textured with the real uploaded garment photo (colour panel for the catalog match — see Section 11 for why), embedded via `st.components.v1.html` | Real cloth simulation is specialist paid software; this gets visual impact without that cost. Built from primitives instead of a downloaded model file specifically to add zero weight to the tight Streamlit Cloud memory budget (Section 7) — it renders entirely client-side in the browser. Camera auto-fits to the figure's actual measured size rather than a hand-tuned position, so nothing gets cropped |
