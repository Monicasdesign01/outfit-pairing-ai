...
Monica to actually deploy via the Streamlit Community Cloud website (needs her own GitHub login - this part cannot be done by Claude). Exact steps given in chat. If it fails on memory, revisit the mitigation options above before assuming something else is wrong.
...
| 2 | Find a garment's dominant colour | OpenCV, k-means clustering (k=3, single largest group, no multi-color detection) + a brightness-first check for neutral colours (2026-09-05) | Simplicity chosen deliberately under time pressure — always returns a usable answer rather than sometimes correctly flagging multi-color but giving nothing actionable. Measured and fixed two real bugs 2026-09-05 (missing "orange" reference, neutral grays matched to a random hue) rather than leaving them as limitations - see Section 11 |
